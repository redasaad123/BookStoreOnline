using Core.Entities;
using Cores.Interfaces;
using Infrastructure;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Identity;
using Microsoft.AspNetCore.Mvc;
using Microsoft.EntityFrameworkCore;
using Web.ViewModel;
using Web.Method_Helper;

namespace Web.Controllers
{
    [Authorize("AccessRoleUser")]
    public class BookMarkController : Controller
    {
        private readonly UserManager<AppUsers> userManager;
        private readonly Methods methods;
        private readonly DbAppContext context;

        public BookMarkController(UserManager<AppUsers> user, Methods methods, DbAppContext context)
        {
            this.context = context;
            userManager = user;
            this.methods = methods;
        }
        public async Task<IActionResult> Index()
        {
            var userId = userManager.GetUserId(HttpContext.User);

            var BookIdInBookmark = await methods.BookmarkBookId(userId);
            var BookBasketId = await methods.BasketBookId(userId);


            var books = BookIdInBookmark.Select(id => new BookMarkViewModel
            {
                Books = context.Book.Select(book => new BookMarkItemViewModel
                {
                    Id = book.Id,
                    NameBook = book.NameBook,
                    Price = book.Price,
                    PdfUrl = book.PdfUrl,
                    PhotoUrl = book.PhotoUrl,
                    Date = book.Date,   
                    IsOffer = book.offer,
                    Discount = book.Discount,
                    IsClickedInBasket = BookBasketId.Contains(book.Id),
                    IsClickedInBookMark = BookIdInBookmark.Contains(book.Id),

                }).Where(x => x.Id == id).FirstOrDefault()
            }).ToList();


            return View(books);
        }
    }
}
